    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        void fireOut()
        {
            ...
            if (isPutOut == false)
            {
                if (counter == 150)
                {
                    firePos.X = rand.Next(0, (GraphicsDevice.Viewport.Width - fire.Width));
                    firePos.Y = rand.Next(GraphicsDevice.Viewport.Height / 2, (GraphicsDevice.Viewport.Height - fire.Height));

                    counter = 0;
                }
            }
            
            if (footRPosition.X >= firePos.X && footRPosition.X <= (firePos.X + fire.Width)
                 && newZ >= firePos.Y && newZ <= (firePos.Y + fire.Height))
            {
                Console.WriteLine("pow pow" + footCount);
                if (footCount < 2)
                {
                    footCount++;
                    firePos.X = rand.Next(0, (GraphicsDevice.Viewport.Width - fire.Width));
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (isPutOut == true)
            {
                spriteBatch.Draw(smoke, firePos, new Color(255, 255, 255, smokeAlpha));
            }
            else
            {
                spriteBatch.Draw(fire, firePos, Color.White);
            }

            spriteBatch.Draw(footR, new Vector2(footRPosition.X, newZ), new Color(255, 255, 255, smokeAlpha));
