...
namespace trackingTest2
{
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        void batHit()
        {
            if ((handPositionR.Y >= shoulderRPosition.Y - 100) && (handPositionR.Y <= shoulderRPosition.Y + 70))
            {
                if (handPositionR.X <= ((shoulderRPosition.X + handDistance / 2) + 50))
                    batVis = true;
                else
                    batVis = false;
            }

            batPos = handPositionR;
            ...
            if (head1Visible == true && batVis == true)
            {
                if (batPos.X >= head1Pos.X && batPos.X <= (head1Pos.X + head1.Width) && batPos.Y >= head1Pos.Y && batPos.Y <= (head1Pos.Y + head1.Height))
                {
                    head1Hit++;
                    head1Pos = new Vector2(rand.Next(0, GraphicsDevice.Viewport.Width - head1.Width),
                        rand.Next((int)headPosition.Y, GraphicsDevice.Viewport.Height / 2));
                    headCount = 0;
                    Console.WriteLine("POW POW" + head1Hit);
                    if (head1Hit == 10)
                        head1Visible = false;
                }
            }
