...
namespace trackingTest2
{
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        void throwBall(GameTime gameTimeFunc)
        {
            for (int i = 0; i < balls; i++)
            {
                scottP = Mouse.GetState();
                credoPos = new Vector2(scottP.X, scottP.Y);
                ...
                if (stopThrow == false)
                    ...
                    {
                        if (curMouseX < 200)
                            throwTime += (float)gameTimeFunc.ElapsedGameTime.TotalSeconds;
                        else
                        {
                            stopThrow = true;
                        }
                ...
                if (stopThrow == true)
                {
                    throwVel = 200.0f / throwTime;

                    time += (float)gameTimeFunc.ElapsedGameTime.TotalSeconds;

                    ballAcc.X = throwVel / 2;
                    //finalBallPos = initBallPos + initBallVel * time + 0.5f * ballAcc * time * time;
                    finalBallPos = initBallPos + new Vector2(throwVel, -50) * time + 0.5f * ballAcc * time * time;

                    if (finalBallPos.Y <= 170)
                    {
                        ballFlip = true;
                    }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            //if (ballVisible == true)
           // {
            spriteBatch.Draw(credo, credoPos, Color.White);

            if(stopThrow == true)
                spriteBatch.Draw(ball, finalBallPos, Color.White);
            //}
