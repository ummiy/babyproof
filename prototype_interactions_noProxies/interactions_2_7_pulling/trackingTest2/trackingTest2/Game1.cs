...
namespace trackingTest2
{
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        void pulling()
        {
            armlength = handPositionR.X - shoulderRPosition.X;

            if (armlength < 100)
            {

                if (handPositionR.X >= towelPos.X && handPositionR.X <= (towelPos.X + towel.Width)
                    && handPositionR.Y >= towelPos.Y && handPositionR.Y <= (towelPos.Y + towel.Height))
                {
                    isTouchT = true;
                }
                ...
                if (isTouchT == true)
                {
                    if (((shoulderRPosition.Z - handPositionR.Z) <= maxRange) && ((shoulderRPosition.Z - handPositionR.Z) >= minRange))
                    {
                        towelPos.X = 400;
                    }
                }

                if (handPositionR.X >= garbagePos.X && handPositionR.X <= (garbagePos.X + garbage.Width)
                    && handPositionR.Y >= garbagePos.Y && handPositionR.Y <= (garbagePos.Y + garbage.Height))
                {
                    isTouchG = true;
                }
